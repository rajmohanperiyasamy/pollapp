...
class BaseSchemataRebuildCommand(BaseCommand):
    """
    ...
    def __new__(cls, *args, **kwargs):
        ...
        """
        # instantiate
        obj = super(BaseSchemataRebuildCommand, cls).__new__(cls, *args, **kwargs)
    ...
    def handle(self, *args, **options):
        ...
        for domain_name in settings.SCHEMATA_DOMAINS:
            ...
            settings.DJAPIAN_DATABASE_PATH=os.path.join( '/home/shabeer/localengine/bm/blackmonk/djapian_data',domain_name)
            print settings.DJAPIAN_DATABASE_PATH
            # call the original command with the args it knows
            call_command(self.COMMAND_NAME, *args, **options)

class Command(BaseSchemataRebuildCommand):
    COMMAND_NAME = 'index'
