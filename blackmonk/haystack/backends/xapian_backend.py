...
class XapianSearchBackend(BaseSearchBackend):
    ...
    def update(self, index, iterable):
        ...
        try:
            ...
            try:  database.close()
            except AttributeError: database = None 
    ...
    def _do_query_facets(self, results, query_facets):
        ...
        return facet_dict
    def _get_suggested_term(self,term,database):
        suggest=database.get_spelling_suggestion(term)
    ...
    def _do_spelling_suggestion(self, database, query, spelling_query):
        ...
        if spelling_query:
            if ' ' in spelling_query:
                suggested_spelling =  ' '.join([self._get_suggested_term(term,database)  for term in spelling_query.split()])
                spelling_query =  ' '.join([term  for term in spelling_query.split()])
                if suggested_spelling==spelling_query:
