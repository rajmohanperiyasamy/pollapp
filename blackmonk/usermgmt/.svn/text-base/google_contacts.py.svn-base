...
class ContactsSample(object):
  ...
  def AllContacts(self):
    ...
    return self.PaginatedFeed(feed, self.PrintContactsFeed)
  
  def PaginatedFeed(self, feed, ctr):
    if not feed.entry:return {}
    data=[]
    for i, entry in enumerate(feed.entry):
        try:
          try:id=entry.id.text.split('/base/')[1]
          except:ids=random.choice('ABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890')
          if not entry.name is None:name='%s'%(entry.name.full_name is None and " " or entry.name.full_name.text)
          else:name='%s'%(entry.title.text)
          data.append({'email':'%s'%entry.email[0].address,'name':'%s'%name,'id':id})
        except:pass
    return data
