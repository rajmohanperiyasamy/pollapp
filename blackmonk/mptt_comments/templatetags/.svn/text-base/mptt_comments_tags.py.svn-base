...
class BaseMpttCommentNode(BaseCommentNode):
    ...
    def __init__(self, ctype=None, object_pk_expr=None, object_expr=None, as_varname=None, comment=None,page=1):
...
class MpttCommentListNode(BaseMpttCommentNode):
    ...
    def get_query_set(self, context):
        ...
        qs = qs.prefetch_related('user').select_related('parent','parent__user').filter(tree_id=root_node.tree_id,level__gte=1,is_removed=False,is_public=True).order_by('-id')
        return qs
    ...
    def render(self, context):
        ...
        context['comments_offset'] = self.offset
...
class MpttCommentListCommunity(BaseMpttCommentNode):
    ...
    def get_query_set(self, context):
        #related = getattr(settings, 'MPTT_COMMENTS_SELECT_RELATED', None)
        qs = super(MpttCommentListCommunity, self).get_query_set(context)
        root_node = self.get_root_node(context)
        qs = qs.prefetch_related('user').select_related('parent','parent__user').filter(level__gte=1,is_removed=False,is_public=True).order_by('-id')
        return qs
...
class MpttCommentListNodeMore(BaseMpttCommentNode):
        ...
        # {% get_whatever for app.model pk as varname %}
            ...
            if tokens[5] == 'page':
                return cls(
                    object_expr = parser.compile_filter(tokens[2]),
                    as_varname = tokens[4],
                    page=parser.compile_filter(tokens[5])
                )
            elif tokens[4] != 'as':
    ...
    def get_query_set(self, context):
        qs = super(MpttCommentListNodeMore, self).get_query_set(context)
        root_node = self.get_root_node(context)
        qs = qs.prefetch_related('user').select_related('parent','parent__user').filter(tree_id=root_node.tree_id,level__gte=1,is_removed=False,is_public=True).order_by('-id')
        return qs
        
    def get_context_value_from_queryset(self, context, qs):
        page=int(self.page.resolve(context))-1
        start=(page*self.offset)
    ...
    def render(self, context):
        ...
        context[self.as_varname] = self.get_context_value_from_queryset(context, qs)
        comments_remaining = self.get_query_set(context).count()
        context['comments_remaining'] = (comments_remaining - self.limit) > 0 and comments_remaining - self.offset or 0
        context['comments_offset'] = self.offset
        context['root_comment'] = self.get_root_node(context)
        context['collapse_levels_above'] = 2
        context['cutoff_level'] = self.cutoff_level
...
class MpttCommentListMoreCommunity(BaseMpttCommentNode):
    ...
    def get_query_set(self, context):
        qs = super(MpttCommentListMoreCommunity, self).get_query_set(context)
        root_node = self.get_root_node(context)
        qs = qs.prefetch_related('user').select_related('parent','parent__user').filter(level__gte=1,is_removed=False,is_public=True).order_by('-id')
        return qs
...
def get_mptt_comment_list_more(parser, token):
    """
    Gets the list of comments for the given params and populates the template
    context with a variable containing that value, whose name is defined by the
    'as' clause.
    ...
    """
    return MpttCommentListNodeMore.handle_token(parser, token)

def get_mptt_comment_list_community(parser, token):
    """
    Gets the list of comments for the given params and populates the template
    context with a variable containing that value, whose name is defined by the
    'as' clause.
    ...
    """
    return MpttCommentListCommunity.handle_token(parser, token)

def get_mptt_comment_list_more_community(parser, token):
    """
    Gets the list of comments for the given params and populates the template
    context with a variable containing that value, whose name is defined by the
    'as' clause.
    ...
    """
    return MpttCommentListMoreCommunity.handle_token(parser, token)
...
register.tag(get_mptt_comment_list_more)

register.tag(get_mptt_comment_list_community)
register.tag(get_mptt_comment_list_more_community)
