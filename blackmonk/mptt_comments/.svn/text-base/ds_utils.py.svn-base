...
def get_business_todays_working_hour(wh_obj):
    try:
        if datetime.now().strftime("%A") == 'Sunday':
            if wh_obj.sun_start:
                work_hours = wh_obj.sun_start.lower()+' - '+wh_obj.sun_end.lower()
            else:work_hours = 'Closed'
        elif datetime.now().strftime("%A") == 'Monday':
            if wh_obj.mon_start:
                work_hours = wh_obj.mon_start.lower()+' - '+wh_obj.mon_end.lower()
            else:work_hours = 'Closed'
        elif datetime.now().strftime("%A") == 'Tuesday':
            if wh_obj.tue_start:
                work_hours = wh_obj.tue_start.lower()+' - '+wh_obj.tue_end.lower()
            else:work_hours = 'Closed'
        elif datetime.now().strftime("%A") == 'Wednesday':
            if wh_obj.wed_start:
                work_hours = wh_obj.wed_start.lower()+' - '+wh_obj.wed_end.lower()
            else:work_hours = 'Closed'
        elif datetime.now().strftime("%A") == 'Thursday':
            if wh_obj.thu_start:
                work_hours = wh_obj.thu_start.lower()+' - '+wh_obj.thu_end.lower()
            else:work_hours = 'Closed'
        elif datetime.now().strftime("%A") == 'Friday':
            if wh_obj.fri_start:
                work_hours = wh_obj.fri_start.lower()+' - '+wh_obj.fri_end.lower()
            else:work_hours = 'Closed'        
        else:
            if wh_obj.sat_start:work_hours = wh_obj.sat_start.lower()+' - '+wh_obj.sat_end.lower()
            else:work_hours = 'Closed'  
    except:
        work_hours = False  
    return work_hours
