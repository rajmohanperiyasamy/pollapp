...
!function($) {
    ...
    var bUiSlCt = function(element, options, e) {
        if (e) {
            e.stopPropagation();
            e.preventDefault();
        }
        ...
        this.options = $.extend({}, $.fn.bUiSlCt.defaults, this.$element.data(), typeof options == 'object' && options);
        ...
        this.val = bUiSlCt.prototype.val;
        this.render = bUiSlCt.prototype.render;
        this.refresh = bUiSlCt.prototype.refresh;
        this.selectAll = bUiSlCt.prototype.selectAll;
        this.deselectAll = bUiSlCt.prototype.deselectAll;
        this.init();
    };
    ...
    bUiSlCt.prototype = {
        ...
        init: function (e) {
            ...
            if (this.$element.attr('class')) {
                this.$newElement.addClass(this.$element.attr('class').replace(/bUiSlCt/gi, ''));
            }

            //If we are multiple, then add the show-tick class by default
            if(this.multiple) {
                 this.$newElement.addClass('show-tick multiple');
            }
        ...
        createDropdown: function() {
            var drop =
                "<div class='bUi-sLcT dropdown'>" +
                    "<button type='button' class='btn dropdown-toggle' data-toggle='dropdown'>" +
                        "<span class='filter-option pLlT dSpL'></span>&nbsp;" +
                        "<i aria-hidden='true' class='bUi-iCn-cRt-tB-12 pLrT dSpL'></i>" +
                    "</button>" +
        ...
        createLi: function() {
            ...
            this.$element.find('option').each(function(index) {
                ...
                var icon = $this.data('icon') !== undefined ? '<i class="'+$this.data('icon')+' mRr5 "></i> ' : '';
                if (icon !== '' && ($this.is(':disabled') || $this.parent().is(':disabled'))) {
                    icon = '<span>'+icon+'</span>';
                }
				var _location = $this.data('location') !== undefined ? $this.data('location') : '';
                ...
                if (_this.options.hideDisabled && ($this.is(':disabled') || $this.parent().is(':disabled'))) {
                    ...
                    if ($this.index() == 0) {
                        ...
                        if ($this[0].index != 0) {
                            _liA.push(
                                ...
                                '<span class="nav-header">'+label+'</span>'+
                                _this.createA(text, "opt " + optionClass, _location )
                                );
                        } else {
                            _liA.push(
                                '<span class="nav-header">'+label+'</span>'+
                         ...
                         _liA.push( _this.createA(text, "opt " + optionClass, _location )  );
                    }
                    ...
                    _liA.push( _this.createA(text, optionClass, _location ) );
                }
            });
        ...
        createA:function(text, classes, location) {
         return '<a tabindex="0" href="'+location+'" class="'+classes+'">' +
                 text +
                 '<i class="bUi-iCn-oK-16 lH1 check-mark" aria-hidden="true"></i>' +
                 '</a>';
        },
        ...
        setSize:function() {
            ...
            if (this.options.size == 'auto') {
                var getSize = function() {
                    ...
                    /*
                    if ((menu.find('li').length + menu.find('span').length) > 2) {
                        minHeight = liHeight*2 + menuExtras - 2;
                    } else {
                        minHeight = 0;
                    }*/
                    minHeight = 0;
                ...
                menuHeight = 25*this.options.size;
        ...
        setDisabled:function(index, disabled) {
            if(disabled) {
                ...
                this.$newElement.find('li').eq(index).removeClass('disabled').find('a').attr('tabindex',0);
            }
        },
        ...
        clickListener: function() {
			...
			this.$element.each(function(index) {
				if(!$(this).data('link') == true){
					_this.$newElement.on('click', 'li a', function(e){
						var clickedIndex = $(this).parent().index(),
							$this = $(this).parent(),
							$select = $this.parents('.bUi-sLcT'),
           ...
           this.$newElement.on('click', 'li.disabled a, li span.nav-header, li .div-contain', function(e) {
                e.preventDefault();
                e.stopPropagation();
                var $select = $(this).parent().parents('.bUi-sLcT');
                $select.find('button').focus();
            });
    ...
    $.fn.bUiSlCt = function(option, event) {
       ...
       var chain = this.each(function () {
            if ($(this).is('select')) {
                var $this = $(this),
                    data = $this.data('bUiSlCt'),
                    options = typeof option == 'object' && option;

                if (!data) {
                    $this.data('bUiSlCt', (data = new bUiSlCt(this, options, event)));
                } else if(options){
    ...
    $(document)
        .data('keycount', 0)
        .on('keydown', '[data-toggle=dropdown], [role=menu]' , bUiSlCt.prototype.keydown)
