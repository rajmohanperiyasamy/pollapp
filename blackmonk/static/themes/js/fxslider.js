;(function($){
	...
	$.fn.fxslider = function(options) {
		var defaults = {
			...
			index,
			fxsliderOn = false,
			html = '<div id="fxslider-outer"><div id="fxslider-Gallery"><div id="fxslider-slider"></div><a id="fxslider-close" class="close"></a></div></div>',
		...
		var setUp = {
			...
			structure: function(){
				$('body').append(html).addClass('fxslider');
				$galleryCont = $('#fxslider-outer');
				$gallery = $('#fxslider-Gallery');
				$slider = $gallery.find('#fxslider-slider');
				var slideList = '';
				if(settings.dynamic == true){
					for(var i = 0; i<settings.dynamicEl.length; i++){
						slideList += '<div class="fxslider-slide"></div>';	
					}	
				}else{
				...
				$slide = $gallery.find('.fxslider-slide');
			},		
			closeSlide: function(){
				var $this = this;
				$('#fxslider-close').bind('click touchend', function(){
					$this.destroy();
				});
			},	
			getWidth: function(){
			...
			touch:function(){
				var xStart,xEnd;
				var $this = this;
				$('.fxslider').bind('mousedown',function(e){
					e.stopPropagation();
					e.preventDefault();
					xStart = e.pageX;
				});
				$('.fxslider').bind('mouseup',function(e){
					e.stopPropagation();
					e.preventDefault();
					xEnd = e.pageX; 
					if(xEnd-xStart>20){
			...
			slideTo : function(){
				...
				if(settings.controls === true && $children.length > 1){
					$gallery.append('<div id="fxslider-action"><a id="fxslider-prev"></a><a id="fxslider-next"></a></div>');
					$prev = $gallery.find('#fxslider-prev');
					$next = $gallery.find('#fxslider-next');	
...
/*					if(this.doCss()){
					...
					if(!this.doCss() && !fxsliderOn){
						$slider.css({ left : (-index*100)+'%' });
						//$slide.eq(index).css('transition','none');
					}
					else if(!this.doCss() && fxsliderOn){
				...
				prevIndex = index;
				fxsliderOn === false ? settings.onOpen.call(this) : settings.onSlideAfter.call(this);
				fxsliderOn = true;
				usingThumb = false;
			},
			destroy : function(){
				...
				fxsliderOn = false;
				clearInterval(interval);
				$('.fxslider').unbind('mousedown');
				$('.fxslider').unbind('mouseup');
