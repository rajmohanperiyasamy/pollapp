...
///// POLL LISTING FILTERING
function filter_content(){
	var ids=[]
	var dataString='';
	var url= $('#id_listpoll_url').val();
...
function show_count(value){
	...
	var count_url = $('#id_polls_status_url').val();
	$.ajax({
	type: "GET",
	url: count_url,
	data: 'status='+value,
	...
	if($('#id_action').val()!='0'){
		...
		if(flag){
			if($('#id_action').val()=='DEL'){
				if(confirm(gettext('Are you sure you want to delete selected poll(s) ?'))){flag_action=true;}
			}
			
			if(flag_action){filter_action(0,'#');}
		}else{
			alert('Please select poll(s)')
		}
	}
}
//// Manage Slug 
