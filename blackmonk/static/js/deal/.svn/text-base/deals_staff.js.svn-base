...
///// DEALS LISTING FILTERING
function filter_content(){
	var ids=[]
	var dataString='';
	var url= $('#id_listing_url').val();
	...
	success: function(data){
			...
			$(".light_box_voucher").colorbox({width: "890",initialWidth: "890", top:"0", initialHeight: "200"});
...
function show_count(value){
	...
	var count_url = $('#id_deal_status_url').val();
	$.ajax({
	type: "GET",
	url: count_url,
	data: 'status='+value,
	...
	success: function(data){
			$('#ajax_loading').hide();
			$('#id_act').html(data.active);
			$('#id_inact').html(data.inactive);
	...
	if($('#id_action').val()!='0'){
		...
		if(flag){
			if($('#id_action').val()=='DEL'){
				if(confirm(gettext('Are you sure you want to delete selected deal(s) ?'))){flag_action=true;}
			}
			if($('#id_action').val()=='P'){
				if(confirm(gettext('Are you sure you want to active selected deal(s) ?'))){flag_action=true;}
			}
			if($('#id_action').val()=='B'){
				if(confirm(gettext('Are you sure you want to inactive selected deal(s) ?'))){flag_action=true;}
			}
			if(flag_action){filter_action(0,'#');}
		}else{
			alert('Please select deal(s)')
		}
	}
}
