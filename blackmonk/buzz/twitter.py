...
' This function escapes the html special quotes and returns it as actual html tags '
...
class TStatus(object):
    ...
    def get_created_at_inseconds(self):
    ...
    def get_relative_createdat(self):
    ...
    def set_image_url(self,profile_image_url):
    ...
    def get_created_at_in_seconds(self):
        '''Get the time this status message was posted, in seconds since the epoch.
    ...
    profile_image_url=property(get_image_url,set_image_url,doc='')
    ...
    relative_created_at = property(get_relative_createdat,
                                 doc='Get a human readable string representing'
                                     'the posting time')
    text = property(get_text, set_text,
                        doc='The status message.')
    location = property(get_location, set_location,
                        doc='The message sent location.')
    from_user = property(get_from_user, set_from_user,
                        doc='The status location.')
    source = property(get_source,set_source,doc='')
    
    created_at_in_seconds = property(get_created_at_in_seconds,
    ...
    def NewFromJsonDict(data):
        ...
        '''
        return TStatus(created_at=data.get('created_at', None),
                      text=data.get('text', None),
                      location=data.get('location',None),
                      source=unescape(data.get('source', None)),
...
class Api(object):
  ...
  ' retrieves list tweets of the user  '
  from buzz.models import TwitterAPISettings
  def retrieve_list_tweets(self,list_name,page,per_page_url):
      try:api_settings = TwitterAPISettings.objects.all()[:1][0]
      except:pass
      
      url  = "http://api.twitter.com/1/"+api_settings.twitter_user+"/lists/"+list_name+"/statuses.json?page="+page+per_page_url
      json = self._FetchUrl(url)
      data = simplejson.loads(json)
      self._CheckForTwitterError(data)
      return [Status.NewFromJsonDict(x) for x in data]
  
  ' retrieves tweets from twitter search '
  
  def retrieve_search_tweets(self,keyword_url,page_url):
      from mybangalore.common.utils import get_global_settings
      global_settings = get_global_settings()
      
      url = 'http://search.twitter.com/search.json?geocode=%.6f'%(float(global_settings.google_map_lat))+'%2C'+'%.6f'%(float(global_settings.google_map_lon))+'%2C30km'+keyword_url+page_url
      #url = 'http://search.twitter.com/search.json?geocode=51.054344%2C-114.075164%2C30km'+keyword_url+page_url
      #url = 'http://search.twitter.com/search.json?geocode='+global_settings.BUZZ_TWITTER_CITY_LAT+'%2C'+global_settings.BUZZ_TWITTER_CITY_LONG+'%2C'+global_settings.twitter_city_km+'km'+keyword_url+page_url
      ...
      for x in data['results']:
          temp = TStatus.NewFromJsonDict(x)
          #temp.user = self.GetUser(urllib.quote(x['from_user']))
          results.append(temp)
      return results 
  
  'retrieve trends from twitter'
