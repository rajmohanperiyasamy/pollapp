...
from payments.paypal.standard.conf import *
from payments.paypal.standard.widgets import ValueHiddenInput, ReservedValueHiddenInput
from payments.paypal.standard.conf import (POSTBACK_ENDPOINT, SANDBOX_POSTBACK_ENDPOINT)
...
class PayPalPaymentsForm(forms.Form):
    def __init__(self, *args, **kwargs):
        ...
        common_conf = CommonConfigure.objects.only('paypal_receiver_email')[:1][0]
        RECEIVER_EMAIL = common_conf.paypal_receiver_email
        self.fields['business'].initial = RECEIVER_EMAIL
    """
    ...
    def render(self):
        return mark_safe(u"""<form action="%s" id="paypalform" method="post">
    %s
    ...
    def sandbox(self):
        return mark_safe(u"""<form action="%s" id="paypalform"  method="post"> %s</form>""" % (SANDBOX_POSTBACK_ENDPOINT, self.as_p()))
        #<!--input type="image" src="%s" border="0" name="submit" alt="Buy it Now" /-->, self.get_image()   
    def get_image(self):
        return {
