...
def people_findByEmail(email):
    """Returns User object."""
    gallery_settings = GallerySettings.get_obj()
    email = gallery_settings.flickr_email
    password = gallery_settings.flickr_password
...
def _doget(method, auth=False, fl_api_key=False, fl_api_secret=False, **params):
    #uncomment to check you aren't killing the flickr server
    #print "***** do get %s" % method
    try:
        gallery_settings = GallerySettings.get_obj()
        if gallery_settings.flickr_api_key:
            API_KEY = gallery_settings.flickr_api_key
        else:
            API_KEY = fl_api_key
        if gallery_settings.flickr_api_secret:
            API_SECRET = gallery_settings.flickr_api_secret
        else:
            API_SECRET = fl_api_secret
    except:
        API_KEY = fl_api_key
        API_SECRET = fl_api_secret
...
def getPublicPhotos(q):
    url = 'http://api.flickr.com/services/feeds/photos_public.gne?%s'%(q)
    #return unmarshal((minidom.parse(urlopen(url))))
    a = urlopen(url)
    return a.read()

def validate_flickr(API_KEY,API_SECRET):
    """Returns a frob that is used in authentication"""
    method = 'flickr.auth.getFrob'
    sig_str = API_SECRET + 'api_key' + API_KEY + 'method' + method
    signature_hash = hashlib.md5(sig_str).hexdigest()
    data = _doget(method, auth=False, fl_api_key=API_KEY.strip(), fl_api_secret=API_SECRET.strip(), api_sig=signature_hash)
    return data.rsp.frob.text
