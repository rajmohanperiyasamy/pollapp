...
class OAuthApi():
    ...
    def GetListTweets(self,list_name,page,per_page_url):
        from buzz.models import TwitterAPISettings    
        try:api_settings = TwitterAPISettings.objects.all()[:1][0]
        except:pass
        try:list_name = list_name.replace(' ','-')
        except:pass
        url  = "https://api.twitter.com/1/"+api_settings.twitter_user+"/lists/"+list_name+"/statuses.json?page="+page+per_page_url
        return self.ApiCall(url, "GET", {}) 
        
    
    def ApiCall(self, call, type="GET", parameters={}):
        json = self._FetchUrl(call, type, parameters)
