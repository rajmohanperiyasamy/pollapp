...
def get_latest_article(number):
    ...
    if latestarticles is None:
        latestarticles = Article.objects.select_related('articlephotos').filter(status='P').only('title', 'slug', 'category', 'created_on', 'published_on', 'album').select_related('category', 'album').order_by('-id')[:number]
...
def get_featured_article(limit):
    ...
    if featuredarticle is None:
        featuredarticle = Article.objects.filter(status='P',featured=True).only('title', 'slug', 'category', 'created_on', 'published_on', 'created_by', 'album').select_related('category', 'created_by', 'album').order_by("?")[:limit]
