...
def filter_banners(request):
    ...
    if search:
        ...
        if len(args) == 0 :banners = BannerAdvertisements.objects.filter(**key).select_related('section','zones','created_by').order_by(sort)
        else:banners = BannerAdvertisements.objects.filter(args,**key).select_related('section','zones','created_by').order_by(sort)
        ...
        if form.is_valid():
            ...
            if payment_settings.online_payment and payment_mode=='online' and not banner_obj:
                ...
                if payment_mode=='online':
                    ...
                    if float(add_banner.temp_amount)>float(tot):
                        return HttpResponseRedirect(reverse('banner_payments_offline_confirm',args=[add_banner.id,add_banner.payment_id]))
                    else:
                        if appreoval_settings.paid_update and banner_obj.status == 'P':
                            add_banner.status = 'P'
                            mail_publish_banner(add_banner)
